...
exports.renderRawPage = renderRawPage;
/**
...
function normalizeDateValue(val, inputType) {
    if (!val)
        return '';
    try {
...
function generateDescription(components) {
    if (!Array.isArray(components))
        return 'Interactive page powered by ClawBoard';
    ...
    const parts = [];
    if (stats)
        parts.push(`${stats} metric${stats > 1 ? 's' : ''}`);
    if (charts)
...
function flattenForDesc(components) {
    const result = [];
    for (const comp of components) {
        result.push(comp);
        if (comp.children)
            result.push(...flattenForDesc(comp.children));
        if (comp.props?.tabs) {
            for (const tab of comp.props.tabs) {
                if (tab.children)
                    result.push(...flattenForDesc(tab.children));
            }
        }
        if (comp.props?.items) {
            for (const item of comp.props.items) {
                if (item.children)
                    result.push(...flattenForDesc(item.children));
            }
        }
    }
    return result;

    ...
    try {
        switch (command) {
            case 'publish': {
                ...
                const body = { title, ttl };
                if (spec) {
                    body.spec = JSON.parse(spec);
                }
                else if (specFile) {
                    const content = fs_1.default.readFileSync(path_1.default.resolve(specFile), 'utf-8');
                    body.spec = JSON.parse(content);
                }
                else if (file) {
                    body.html = fs_1.default.readFileSync(path_1.default.resolve(file), 'utf-8');
                }
                else if (htmlArg) {
                    body.html = htmlArg;
                }
                else {
                    body.html = fs_1.default.readFileSync('/dev/stdin', 'utf-8');
                }
                if (deliver) {
            ...
            case 'deliver': {
                ...
                if (!id) {
                    console.error('Usage: clawboard deliver <page-id> [platform]');
                    process.exit(1);
                }
            ...
            case 'delete': {
                const id = args[1];
                if (!id) {
                    console.error('Usage: clawboard delete <id>');
                    process.exit(1);
                }
            ...
            case 'start': {
                ...
                if (noTunnel)
                    process.env.CLAWBOARD_NO_TUNNEL = '1';
                require('./server');
                break;
            }
            default:

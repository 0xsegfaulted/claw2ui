...
const renderDivider: ComponentRenderFn = () => `<hr class="my-4 border-gray-200 dark:border-gray-700">`;
...
const renderModal: ComponentRenderFn = (comp, renderChild) => {
  const p = comp.props || {};
  const triggerChild = comp.children?.[0] ? renderChild(comp.children[0]) : `<button class="px-4 py-2 bg-blue-600 text-white rounded-lg">${esc(p.triggerLabel || 'Open')}</button>`;
...
/** Create a fully populated component registry for the Classic theme. */
export function createClassicRegistry(): ComponentRegistry {
...
const componentRegistry = createClassicRegistry();
